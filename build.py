...
<body>
  ...
  <script>
    ...
    const battleReward = (level, type) => Math.round(level[type] * upgradeMultiplier('battleprize') * (hasRelic('idol') ? 1.5 : 1) * (hasRelic('warchain') ? 1.75 : 1));
    ...
    const packCost = (storeId = selectedStore) => paidMode() ? +(meta().price * store(storeId).priceMult * UPGRADES[3].mult[S.up.whole] * (hasRelic('coupon') ? .95 : 1) * (hasRelic('smugglermap') ? .8 : 1) * (hasRelic('idol') ? 1.15 : 1) * (S.mode === 'hard' ? HARD_PACK_MULT : 1)).toFixed(2) : 0;
    ...
    const minPackCost = () => Math.min(...Object.values(SET_META).map(s => +(s.price * STORES.black.priceMult * UPGRADES[3].mult[S.up.whole] * (hasRelic('coupon') ? .95 : 1) * (hasRelic('smugglermap') ? .8 : 1) * (hasRelic('idol') ? 1.15 : 1) * (S.mode === 'hard' ? HARD_PACK_MULT : 1)).toFixed(2)));
    ...
    function rollHitRarity(storeId = selectedStore) {
      const entries = Object.entries(HIT_W).filter(([r]) => BY[r] && BY[r].length).map(([r, w]) => [r, CHASE.has(r) ? w * luckMult() * (hasRelic('luckycoin') ? 1.5 : 1) * (hasRelic('cursedclover') ? 2 : 1) * store(storeId).luckMult * (S.kissBoost ? KISS_LUCK_MULT : 1) * (S.shakeBoost ? shakeLuckMult() : 1) * (S.mode === 'hard' ? HARD_CHASE_MULT : 1) : w]);
    ...
    function pullPack(storeId = selectedStore, fake = false) {
      const used = new Set();
    ...
    function openPack() {
      ...
      const fakeChance = Math.min(1, store(storeId).fakeChance + (hasRelic('crown') ? .15 : 0) + (hasRelic('smugglermap') ? .20 : 0));
      ...
      S.last = { pull, cost, total, store: storeId, fake };
    ...
    function render(pull, cost, total) {
      ...
      $('summary').innerHTML = paidMode()
        ? `${purchaseStore.icon} ${purchaseStore.name} • ${S.last && S.last.fromBox ? 'Box pack' : `Pack ${money(cost)}`} → cards sold ${money(total)} ${fakeWarning}<div class="delta ${delta >= 0 ? 'up' : 'down'}">${delta >= 0 ? '+' : ''}${money(delta)}</div><small style="opacity:.7">hit: ${hit.c.n} (${hit.c.r})</small>`
    ...
    function hud() {
      ...
      updateShakeButton();
      updateQuotaHud();
      if (paid && !totalBoxPacks() && S.bank < minPackCost() && !(ACTIVE_BATTLE && !ACTIVE_BATTLE.finished)) setTimeout(bust, 1200);
    }
    ...
    const storeButtons = target => {
      $(target).innerHTML = Object.entries(STORES).map(([id, s]) => `<button class="store-choice" data-store="${id}"><b>${s.icon} ${s.name}</b><small>${s.desc}</small></button>`).join('');
    };
