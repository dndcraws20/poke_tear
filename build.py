...
<body>
  ...
  <script>
    ...
    const binderValue = item => item.fake ? 0 : +(item.value * (Date.now() - item.keptAt >= BINDER_GROW_MS ? binderGrowthMult() : 1)).toFixed(2);
    ...
    const packCost = (storeId = selectedStore) => paidMode() ? +(meta().price * store(storeId).priceMult * UPGRADES[3].mult[S.up.whole] * (S.mode === 'hard' ? HARD_PACK_MULT : 1)).toFixed(2) : 0;
    ...
    const minPackCost = () => Math.min(...Object.values(SET_META).map(s => +(s.price * STORES.black.priceMult * UPGRADES[3].mult[S.up.whole] * (S.mode === 'hard' ? HARD_PACK_MULT : 1)).toFixed(2)));
    ...
    function rollHitRarity(storeId = selectedStore) {
      const entries = Object.entries(HIT_W).filter(([r]) => BY[r] && BY[r].length).map(([r, w]) => [r, CHASE.has(r) ? w * luckMult() * store(storeId).luckMult * (S.kissBoost ? KISS_LUCK_MULT : 1) * (S.shakeBoost ? shakeLuckMult() : 1) * (S.mode === 'hard' ? HARD_CHASE_MULT : 1) : w]);
    ...
    function rollGrade(storeId = selectedStore) {
      ...
      const rolls = UPGRADES[11].mult[recheckLevel] + store(storeId).psaRerolls;
    ...
    function openPack() {
      ...
      const fake = Math.random() < store(storeId).fakeChance;
