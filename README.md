...
- **Persistent card binder** — keep up to eight cards across runs, view them in a large two-column gallery, tap any card for a full-size inspection, grade cards with PSA inside the binder, or sell them into an active money-mode run. The one-minute value change remains intentionally hidden and updates without resetting or blinking the cards.
- **Three-card Binder battles** — build a permanent deck from three Binder cards, train them through ten levels, and fight automatic HP-and-damage battles where every card uses its strongest attack. The battle screen shows both three-card teams facing each other. Tougher Easy, Normal, Hard, and brutal Impossible trainers award up to $3,000 in single battles and $20,000 in tournaments. Single battles have a saved 10-second cooldown; tournaments have a saved 30-second cooldown. Knocked-out cards always remain in the Binder.
